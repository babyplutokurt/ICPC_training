...
def fancyPairPair(pairs, k1, k2):
    ...
    left = 0
    while left < right:
        soft_cap = k2 - pairs[left][1]
        for i in range(left + 1, right + 1):
            if pairs[i][1] <= soft_cap:
                # print(pairs[left], pairs[i])
                res += 1
        left += 1
        for i in range(right, left - 1, -1):
            if pairs[i][0] <= k1 - pairs[left][0]:
                right = i
                break
    # print("result: ", res)
    return res
...
print(binarySearch(lst, 6, 0))
fancyPairPair(lst, 9, 36)
"""
lst3 = []
start1 = time.time()
...
print(fancyPairPair(lst3, 9999, 9999))
